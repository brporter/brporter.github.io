...
document.addEventListener('keydown', (e) => {
    if (document.getElementById('emulator-modal').classList.contains('active')) {
        return; // Don't track Konami code when emulator is active
    }
    
    // Use e.key for letter keys, e.code for arrow keys
    const key = e.code.startsWith('Arrow') ? e.code : e.key.toLowerCase();
    
    console.log('Key pressed:', e.key, '| Code:', e.code, '| Using:', key);
    console.log('Expected:', KONAMI_CODE[konamiIndex], '| Index:', konamiIndex);
    
    if (key === KONAMI_CODE[konamiIndex]) {
        konamiIndex++;
        console.log('✓ Match! Progress:', konamiIndex + '/' + KONAMI_CODE.length);
        ...
        if (key === KONAMI_CODE[0]) {
            konamiIndex = 1;
            console.log('✗ Reset, but matched start. Progress: 1/' + KONAMI_CODE.length);
        } else {
            konamiIndex = 0;
            console.log('✗ No match. Reset to 0');
        }
    }
});
